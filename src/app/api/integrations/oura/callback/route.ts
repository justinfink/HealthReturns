...
import { exchangeCodeForToken, storeOuraConnection } from "@/lib/integrations/oura/oauth"
import { getOuraClient } from "@/lib/integrations/oura/client"
import { prisma } from "@/lib/db/prisma"

// GET /api/integrations/oura/callback - Handle Oura OAuth callback
export async function GET(request: NextRequest) {
  try {
    ...
    console.log("Oura callback received:", {
      hasCode: !!code,
      state,
      error,
      url: request.url,
    ...
    if (error) {
      ...
      return NextResponse.redirect(
        new URL("/employee/connect?error=oura_denied", request.url)
      )
    }
    ...
    // Build the redirect URI (must match exactly what was sent to Oura)
    const origin = request.headers.get("origin") || request.headers.get("host")
    const protocol = request.headers.get("x-forwarded-proto") || "https"
    const appUrl = origin?.startsWith("http") ? origin : `${protocol}://${origin}`
    const redirectUri = `${appUrl}/api/integrations/oura/callback`
    ...
    await storeOuraConnection(memberId, tokenResponse)
    console.log("Oura connection stored successfully")

    // Trigger initial sync (limited to avoid rate limits - just last 7 days)
    console.log("Triggering initial Oura sync...")
    try {
    ...
    return NextResponse.redirect(
      new URL("/employee/connect?connected=oura", request.url)
    )
  } catch (error: unknown) {
    const errorMessage = error instanceof Error ? error.message : String(error)
    const errorStack = error instanceof Error ? error.stack : undefined
    console.error("Error in Oura OAuth callback:", {
